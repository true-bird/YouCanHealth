...
.redtext{
	...
	<link rel="stylesheet" type="text/css" href="<c:url value='/css/community.css?after'/>"/> <!-- (insert) -->
</head>
<body>
	<hr>
	<table style="background-color:#364156;">
		...
		<tr>
			<th colspan="3" style="font-size: 30px;">제목: ${communityDto.title}</th>
		<tr>
			<th>내용</th>
			<th>
				<textarea name="content" cols=100 rows=20 style="font-size: 20px;" readonly="readonly">${communityDto.content}</textarea>
			</th>
		</tr>
	...
	<hr>
	댓글
	<table class="type08" >	<c:choose>
			<c:when test="${fn:length(list)>0 }">
				<tr>
				<td style="border-top: none; border-bottom: none; border-left: hidden; border-right: none;">&nbsp</td>
					<th>아이디</th>
					<th>내용</th>
					<th>생성일자</th>
					<td style="border-top: none; border-bottom: none; border-left: none; border-right: none;">&nbsp</td>
				</tr>
				<hr>
			</c:when>
		</c:choose>
		...
		<c:forEach var="user" items="${list}" varStatus="status">
			...
			<c:choose>
				<c:when test="${user.id eq name}">
					<tr>
					<td style="border-top: none; border-bottom: none; border-left: hidden; ">&nbsp</td>
						<td>${user.id}</td>
						<td>${user.content}</td>
				...
				<c:otherwise>
					<tr><td style="border-top: none; border-bottom: none; border-left: hidden;">&nbsp&nbsp&nbsp&nbsp&nbsp</td>
						<td>${user.id}</td>
						<td>${user.content}</td>
						<td>${user.commentdate}</td>
					</tr>
	...
	<form action="<%=contextPath%>/community/createComment" method="post">
		<table>
			<tr>
				<td>댓글 달기:<input type="hidden" value="<%=id%>"
					name="id"></td>
